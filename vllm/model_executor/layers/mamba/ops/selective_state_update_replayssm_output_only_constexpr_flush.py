...
def _replayssm_output_only_kernel(
    ...
    # Buffer cursor (number of cached tokens so far). IS_FLUSH is specialized
    # per launch so Triton can compile separate non-flush and flush kernels.
    write_pos = tl.load(write_pos_ptr + pid_b).to(tl.int64)
...
def selective_state_update_replayssm_output_only_constexpr_flush(
    state: torch.Tensor,
    x: torch.Tensor,
    dt: torch.Tensor,
    A: torch.Tensor,
    ...
    """Baseline ReplaySSM op specialized by a launch-wide flush flag.

    This experimental entry point intentionally only supports the unquantized
    cache layout. ``is_flush`` is a Python bool so Triton can compile separate
    non-flush and flush kernels for register/occupancy analysis.
    """
    ...
    if quant_mode != "none":
        raise NotImplementedError(
            "constexpr-flush ReplaySSM experiment only supports quant_mode='none'"
        )
    quant_mode_id = _REPLAYSSM_QUANT_NONE
